...
def plot_pixel_errors(res0, res1):
    ...
    ax1.plot(res0[0, :], res0[1,:], 'rx')#, markersize=4)#, mew=0.4)
    ax1.plot(res1[0, :], res1[1, :], 'bx')#, markersize=4)#, mew=0.4)
    ax2.hist(n0, bins = 100, color = "skyblue")
    ax2.hist(n1, bins = 100, color = "red", alpha=0.5)
    res0 = res0.reshape(2, -1)
    res1 = res1.reshape(2, -1)
    err0 = np.std(res0, axis=1)
    err1 = np.std(res1, axis=1)
    print('reprojection error0: {} ;\nreprojection error1: {}'.format(err0, err1))
    print('reprojection error0: {} ; reprojection error1: {}'.format(np.linalg.norm(err0), np.linalg.norm(err1)))
    plt.show()
    cv.waitKey(0)
